...
def train(config, conf_id, seed):
    ...
    exp_name = f"{conf_id}_sae_downstream_{config["data"]["dataset_name"]}_{seed}"
    ...
    sae_index = config["sae_index"]
    checkpoint = config["checkpoint"]
    num_classes = config["num_classes"]
    model = SaeProbe(config, checkpoint=checkpoint, sae_index=sae_index, num_classes=num_classes, device=device)
...
if __name__ == "__main__":
    ...
    if args.dataset == "esc50":
        ...
        from audIBle.config.sae_downstream_vocalset import conf, common_parameters
    else: 
